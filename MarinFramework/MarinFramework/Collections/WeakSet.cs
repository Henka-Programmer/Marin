...
namespace MarinFramework.Collections
{
    ...
    {
        ISet<WeakReference<T>> store = new HashSet<WeakReference<T>>();
        ...
        {
            return store.Add(new WeakReference<T>(item));
        }
        ...
        {
            return store.Any(x => x.TryGetTarget(out T t) && t == item);
        }
